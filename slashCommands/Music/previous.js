    ...
    execute(client, interaction, args) {
        ...
        if (voiceChannelSize > 2) {
            ...
            if (subscription.metadata.voteSkipList.includes(interaction.user.id)) {
                return interaction.reply(client.emotes.error + " **You already voted to skip to the previous song** (" + subscription.metadata.voteSkipList.length + "/" + requiredVotes + " people)");
            }
            ...
            if (subscription.metadata.voteSkipList.length >= requiredVotes) {
                ...
                interaction.reply(client.emotes.previous + " **Previous**");
            }
            else interaction.reply("**Skip to the previous song?** (" + subscription.metadata.voteSkipList.length + "/" + requiredVotes + " people)");
        }
        else {
            subscription.previous();
            interaction.reply(client.emotes.previous + " **Previous**");
