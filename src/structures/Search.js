    ...
    switch (queryType) {
        case "youtube-video": {
            try {
                ...
                if (!trackInfo) return message.channel.send(Util.emojis.error + " **Could not find that link**");
                ...
                console.log(ex)
                return message.channel.send(Util.emojis.error + " **Error: Searching:** `" + ex.message + "`");
            }
        }
        break;
            ...
            case "youtube-playlist": {
                try {
                    ...
                    if (!playlist) return message.channel.send(Util.emojis.error + " **Could not find that link**");
            ...
            case "youtube-video-keywords": {
                try {
                    trackInfo = await ytsr.searchOne(query)
                    if (!trackInfo) return message.channel.send(Util.emojis.error + " **No results found on YouTube for** `" + query + "`");
